# FactR knowledge

> **Status:** Dieses Projekt wird derzeit nicht aktiv weiterentwickelt.

FactR knowledge ist ein Desktop-Werkzeug zur strukturierten Analyse komplexer Rechercheprozesse. Es entstand aus dem Wunsch heraus, Informationen aus unterschiedlichen Quellen nicht nur zu sammeln, sondern deren Zusammenhänge, Herkunft und Argumentationsketten nachvollziehbar darzustellen.

Mit dem zunehmenden Einsatz künstlicher Intelligenz wurde es immer schwieriger, den Überblick über Quellen, Aussagen und deren Vertrauenswürdigkeit zu behalten. Dieses Projekt verfolgt deshalb einen anderen Ansatz als klassische Notizprogramme: Informationen sollen analysiert, miteinander verknüpft und visuell nachvollziehbar gemacht werden.

## Motivation

Bei umfangreichen Recherchen entstehen häufig Probleme wie:

* Informationen stammen aus zahlreichen Quellen.
* Aussagen widersprechen sich.
* Die ursprüngliche Quelle geht verloren.
* Eigene Gedanken vermischen sich mit fremden Aussagen.
* Ursache-Wirkungs-Beziehungen werden unübersichtlich.

FActR knowledge soll dabei helfen, diese Informationen systematisch zu strukturieren und transparent zu dokumentieren.

## Projektziele

* Quellen zentral verwalten
* Eigene Notizen klar von Quellen trennen
* Aussagen miteinander verknüpfen
* Argumentations- und Kausalketten aufbauen
* Informationen visuell darstellen
* Recherchen langfristig nachvollziehbar dokumentieren
* KI als Werkzeug einsetzen, ohne die Herkunft von Informationen aus dem Blick zu verlieren

## Geplante Funktionen

### Quellenverwaltung

* Verwaltung beliebiger Quellen
* Metadaten (Autor, Datum, URL usw.)
* Tagging und Kategorisierung
* Bewertung von Quellen

### Analyse

* Verknüpfen einzelner Aussagen
* Aufbau logischer Beziehungen
* Visualisierung von Zusammenhängen
* Erkennen von Argumentationsmustern

### Wissensstruktur

* Eigene Gedanken separat dokumentieren
* Beziehungen zwischen Quellen und Notizen herstellen
* Nachvollziehbare Entwicklung eines Rechercheprozesses

### KI-Unterstützung

* Erstellung und Verwaltung von Prompts
* Wiederverwendbare Prompt-Sammlungen
* Unterstützung bei der Analyse großer Informationsmengen

## Architektur

FactR knowledge wurde modular aufgebaut.

* Wissensverwaltung
* Modulverwaltung
* SourceAnalyzer
* PromptCreator
* PromptCollection

Neue Module sollen sich unabhängig voneinander entwickeln und später leicht ergänzen lassen.

## Technologien

Geplant bzw. verwendet wurden unter anderem:

* Python
* PyQt
* SQLite
* modulare Architektur

## Projektstatus

Die Entwicklung ist aktuell pausiert.

Durch die intensive beschäftigung bilden KernIdeen von FactR knowledge heute eine wichtige Grundlage für mein Wissensmanagement und Recherche.

## Lizenz

Dieses Projekt steht unter der MIT-Lizenz.
